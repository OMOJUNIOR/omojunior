...
<div class="w-full max-w-4xl mx-auto overflow-hidden bg-white divide-y divide-gray-300 rounded-lg whitespace-nowrap">
    <div class="relative mt-1">
        ...
        <form method="get" action="{{route('phone.searchPhone')}}" class="my-2 form-inline my-lg-0">

            <input
                class="justify-center pl-8 border border-gray-300 rounded-lg form-control mr-sm-2 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
    ...
    <div class="">
        <div class='w-full overflow-x-auto'>
            <table
                class='w-full max-w-4xl mx-auto overflow-hidden bg-white divide-y divide-gray-300 rounded-lg whitespace-nowrap'>
                    ...
                    <tr>
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-3">
                                <div class="inline-flex w-10 h-10"> <img class='object-cover w-10 h-10 rounded-full'
                                        alt='User avatar' src='{{asset('/storage/'.$contact->getImage())}}' /> </div>
                                <div>
                                    <a class="hover:text-blue-600" href="{{route('phone.show',$contact->getId())}}">
                                        {{$contact->getName()}}</a>
                        ...
                        <td class="px-6 py-4">
                            ...
                            <p class="text-sm font-semibold tracking-wide text-gray-500"> Job: {{$contact->getJob()}}
                            </p>
                        </td>
                        <td class="px-6 py-4 text-center"> <span
                                class="w-1/3 px-2 pb-1 text-sm font-semibold text-white bg-green-600 rounded-full">
                                {{$contact->getRelationship()}} </span> </td>
                        <td class="px-6 py-4 text-center"> {{$contact->getCountry()}} </td>
                        <td class="px-6 py-4 text-center"> <a href="{{route('phone.edit',$contact->getId())}}"
                                class="text-purple-800 hover:underline">Edit</a><br>
                            <form method="post" action="{{route('phone.destroy',$contact->getId())}}">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="px-2 space-y-2 font-semibold text-red-600 bg-white border border-red-400 rounded shadow dark:text-red-500 hover:bg-gray-100">Delete</button>
                            </form>
                        </td>
                    </tr>
                </tbody>
